...
// PUT /posts/:id -> app.controller.posts.updateMany
// DELETE /posts/:id -> app.controller.posts.deleteMany
...
class HomeController extends HttpController {
  ...
  async destroy() {
    ...
    try {
      ...
      const res = await ctx.service.posts.deleteMany({
        _id: postsId,
      });
      await this.success(res);
    } catch (err) {
  ...
  async update() {
    ...
    try {
      ...
      const res = await ctx.service.posts.updateMany({ // 1. 查询条件 2.更新的数据
        _id: postsId,
      }, {
        $set: { // 约束
          ...requestBody,
