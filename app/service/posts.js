...
class PostsService extends Service {
  ...
  async deleteMany(data) { // 1.查询的条件
    const res = await this.ctx.model.Posts.deleteMany(data);
    return res;
  }

  async updateMany(findData, data) { // 1. 查询条件 2. 修改数据源
    const res = await this.ctx.model.Posts.updateMany(findData, data);
    return res;
  }
}
